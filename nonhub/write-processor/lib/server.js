...
import { injectAirportDatabase, injectEntityStateManager, LOCAL_API_SERVER } from '@airport/tower';
...
async function handleRequest(request, reply) {
    ...
    let response = await localApiServer.handleRequest(request);
    response = {
        ...response,
        received: true
    };
    const ecryptedMessage = await encryptString(JSON.stringify(response), masterKey);
