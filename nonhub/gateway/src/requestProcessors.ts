...
export function processReadRequest(
    request,
    reply,
    currentReadBatch: ReadRequestBatch
) {
    let readRequest: ReadRequest = request.body as ReadRequest
    if (!readRequest) {
...
export function processSearchRequest(
    request,
    reply,
    currentSearchBatch: SearchRequestBatch
) {
...
async function processWriteRequestAsync(
    request,
    reply,
    masterKey,
    requestHost
) {
    let writeRequest: WriteRequest = request.body as WriteRequest
    if (!writeRequest) {
        reply.send({ received: false })
        return
    }
    ...
    try {
        const ecryptedMessage = await encryptString(
            JSON.stringify(writeRequest), masterKey)
