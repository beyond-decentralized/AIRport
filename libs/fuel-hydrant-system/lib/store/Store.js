...
async function setStoreDriver(storeType) {
    ...
    switch (storeType) {
        case ground_control_1.StoreType.SQLITE_CORDOVA:
            const webSqlDriverFile = await Promise.resolve().then(() => require('../../db/websql/src/WebSqlDriver'));
            StoreDriver = webSqlDriverFile.WebSqlDriver;
            break;
        case ground_control_1.StoreType.SQLJS:
            const sqlJsDriverFile = await Promise.resolve().then(() => require('../../db/sqljs/src/SqlJsDriver'));
            StoreDriver = sqlJsDriverFile.SqlJsDriver;
            break;
        default:
            throw new Error(`Unsupported StoreType: ${storeType}`);
    }
    const sqLiteSequenceGeneratorFile = await Promise.resolve().then(() => require('../../db/sqlite/src/SqLiteSequenceGenerator'));
