import { DbEntity, IQEntityInternal, JoinTreeNode, QRelation }     from "../../../../apis/air-control/lib/index";
import { JSONEntityFieldInOrderBy, JSONFieldInOrderBy, SortOrder } from "../../../../apis/ground-control/lib/index";
		...
		while (
			...
			orderBy = orderBy.filter((orderByField) => {
				...
				if (this.isForParentNode(currentJoinNode, orderByField)) {
					throw `Found out of order entry in Order By [${orderByDbEntity.schema.name} - ${orderByDbEntity.name}.${dbColumn.name}].  Entries must be ordered hierarchically, in breadth-first order.`;
				}
				if (orderByField.si !== dbEntity.schema.index || orderByField.ti !== dbEntity.index) {
					return true;
				}
