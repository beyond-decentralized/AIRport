...
class UpdateManager {
    async updateValues(portableQuery, actor) {
        ...
        const [airDb, fieldUtils, historyManager, offlineDataStore, operHistoryDuo, queryUtils, recHistoryDuo, recHistoryNewValueDuo, recHistoryOldValueDuo, repositoryManager, repoTransHistoryDuo, schemaUtils, sequenceGenerator, storeDriver, transactionManager] = await di_1.container(this).get(air_control_1.AIR_DB, air_control_1.FIELD_UTILS, tokens_1.HISTORY_MANAGER, tokens_1.OFFLINE_DELTA_STORE, holding_pattern_1.OPER_HISTORY_DUO, air_control_1.QUERY_UTILS, holding_pattern_1.REC_HISTORY_DUO, holding_pattern_1.REC_HIST_NEW_VALUE_DUO, holding_pattern_1.REC_HIST_OLD_VALUE_DUO, tokens_1.REPOSITORY_MANAGER, holding_pattern_1.REPO_TRANS_HISTORY_DUO, air_control_1.SCHEMA_UTILS, check_in_1.SEQUENCE_GENERATOR, ground_control_1.STORE_DRIVER, terminal_map_1.TRANSACTION_MANAGER);
...
`;
    ...
    async addNewValueHistory(jsonUpdate, dbEntity, recordHistoryMapByRecordId, systemWideOperationId, repositorySheetSelectInfo, errorPrefix, airDb, recHistoryDuo, recHistoryNewValueDuo, fieldUtils, queryUtils, storeDriver) {
        ...
        const queryFacade = await di_1.container(this).get(air_control_1.QUERY_FACADE);
...
di_1.DI.set(tokens_1.UPDATE_MANAGER, UpdateManager);
