...
export class ServiceBus
	...
	on(
		...
		return container(this).getSync(BUS_HOOKS).on(action as any, callback as any);
	}
		...
		try {
			...
			if (message) {
				...
				try {
					...
					await Promise.all(container(this).getSync(BUS_HOOKS).error.map(callback => callback(
