...
import {MessageFromTMVerifierToken}    from "../../../InjectionTokens";
import {
	TerminalPassword,
	TerminalId,
...
export interface IMessageFromTMVerifier {
	...
	verifyMessage(
		...
		maxAllRepoChangesLength: number,
		maxSingleRepoChangeLength: number
	): [MessageFromTMError, any] | [MessageFromTMError, any, any];

}

@Service(MessageFromTMVerifierToken)
export class MessageFromTMVerifier
	extends AbstractCommonMessageVerifier
	implements IMessageFromTMVerifier {
	...
	verifyMessage(
		...
		if (error) {
			return <[MessageFromTMError, any]>error;
		}
		...
		const messageType: MessageFromTMContentType = message[1];

		switch (messageType) {
			case MessageFromTMContentType.DATA_TRANSFER:
				return this.verifyDataTransferMessage(
					<SerializedDataTransferMessageFromTM>message,
					maxAllRepoChangesLength,
					maxSingleRepoChangeLength);
			case MessageFromTMContentType.SYNC_VERIFICATIONS:
				return this.verifySyncVerificationMessage(
					<SerializedSyncVerificationMessageFromTM>message
				);
	...
	verifyDataTransferMessage(
		serializedataTransferMessage: SerializedDataTransferMessageFromTM,
		maxAllRepoTransBlocksLength: number,
		maxSingleRepoTransBlockLength: number
	): [MessageFromTMError, any] | [MessageFromTMError, any, any] {
		...
		const serializedTerminalCredentials: SerializedTerminalCredentials = serializedataTransferMessage[2];
		...
		const agtTerminalId: TerminalId = serializedTerminalCredentials[0];
		...
		const terminalPassword: TerminalPassword = serializedTerminalCredentials[1];
		...
		let error;
		repositoryUpdateRequests.some(
			(
				repositoryUpdateRequest: SerializedRepositoryUpdateRequest,
				index: number
			) => {
	...
	verifySyncVerificationMessage(
		syncVerificationMessage: SerializedSyncVerificationMessageFromTM
	): [MessageFromTMError, any] | [MessageFromTMError, any, any] {
