		...
		for (const domain in jsonSchemaMap) {
			...
			for (const schemaName in domainMap) {
				...
				if (!ownSchema) {
					if (failOnMissingMappings) {
						throw new Error(
							`Schema '${schemaName}' is not yet available for relation linking.`);
					}
					continue;
				}
				...
				for (const index in jsonSchemaVersion.referencedSchemas) {
					...
					if (!referencedDbDomain) {
						if (failOnMissingMappings) {
							throw new Error(
								`Domain '${schemaReference.domain}' is not yet available for relation linking.`);
						}
						continue;
					}
					...
					if (!referencedSchema) {
						if (failOnMissingMappings) {
							throw new Error(
								`Schema '${referencedSchemaName}' is not yet available for relation linking.`);
						}
						continue;
					}
					// FIXME: find a way to get the right schema version once versioning is added
					...
					for (const relationIndex in mapForEntity) {
						...
						for (const columnIndex in mapForRelation) {
							...
							if (!oneSchema) {
								if (failOnMissingMappings) {
									throw new Error(
										`Schema '${relationColumnReference.schemaName}' is not yet available for relation linking.`);
								}
								break;
							}
							...
							if (!oneRelation) {
								throw new Error(
									`Relation '${relationColumnReference.schemaName}.${oneEntity.name} - ${relationColumnReference.relationIndex}' is not defined.`);
							}
							const oneColumn = oneEntity.columns[relationColumnReference.columnIndex];
							if (!oneColumn) {
								throw new Error(
									`Column '${relationColumnReference.schemaName}.${oneEntity.name} - ${relationColumnReference.columnIndex}' is not defined.`);
							}
							const manyColumn = oneEntity.columns[columnIndex];
							if (!manyColumn) {
								throw new Error(
									`Column '${schemaName}.${oneEntity.name} - ${columnIndex}' is not defined.`);
							}
