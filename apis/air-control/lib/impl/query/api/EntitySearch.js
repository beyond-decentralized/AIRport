...
class EntitySearch extends EntityLookup_1.EntityLookup {
    ...
    graph(rawGraphQuery) {
        ...
        return this.dbFacade.entity.search(this.dbEntity, entityQuery, ground_control_1.QueryResultType.ENTITY_GRAPH, cacheForUpdate);
    }
    tree(rawTreeQuery) {
        let entityQuery = this.utils.Entity.getEntityQuery(rawTreeQuery);
        const cacheForUpdate = this.cleanNextCallState();
        return this.dbFacade.entity.search(this.dbEntity, entityQuery, ground_control_1.QueryResultType.ENTITY_TREE, cacheForUpdate);
    }
}
