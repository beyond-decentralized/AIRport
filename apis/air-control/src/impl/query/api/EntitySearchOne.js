...
export class EntitySearchOne extends EntityLookup {
    ...
    graph(rawGraphQuery) {
        ...
        return this.dbFacade.entity.searchOne(this.dbEntity, entityQuery, QueryResultType.ENTITY_GRAPH, cacheForUpdate);
    }
    tree(rawTreeQuery) {
        let entityQuery = this.utils.Entity.getEntityQuery(rawTreeQuery);
        const cacheForUpdate = this.cleanNextCallState();
        return this.dbFacade.entity.searchOne(this.dbEntity, entityQuery, QueryResultType.ENTITY_TREE, cacheForUpdate);
    }
}
